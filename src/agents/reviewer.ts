...
import type { AgentMessage, ReviewArtifactPayload, ReviewResultPayload } from '../types/message.types.js';
...
const SECURITY_PATTERNS = [
  ...
  { pattern: /innerHTML\s*=/g, message: 'Direct innerHTML assignment may cause XSS' },
  ...
  { pattern: /process\.env\./g, message: 'Environment variable access (verify it is necessary)', severity: 'info' as const },
];

// Style patterns to check
const STYLE_PATTERNS = [
  { pattern: /console\.log/g, message: 'Remove console.log before production', severity: 'warning' as const },
  { pattern: /\/\/\s*TODO/gi, message: 'TODO comment found', severity: 'info' as const },
  { pattern: /any\s*[;,)>]/g, message: 'Avoid using "any" type', severity: 'warning' as const },
  { pattern: /!important/g, message: 'Avoid !important in CSS', severity: 'warning' as const },
];
...
export class ReviewerAgent extends BaseAgent {
  ...
  private async handleReviewArtifact(
    ...
    for (const artifact of artifacts) {
      ...
      suggestions.push(...this.generateSuggestions(artifact.content, artifact.path));
    }
    ...
    this.log(`Review complete: ${hasBlockingErrors ? 'REJECTED' : 'APPROVED'}, ${allErrors.length} issues found`);
  }
   ...
   */
  private checkSecurityPatterns(content: string, filePath: string): TaskError[] {
    const errors: TaskError[] = [];
