...
export class StateMachine {
  ...
  private listeners: Map<string, Array<(from: SessionStatus, to: SessionStatus) => void>>;
  ...
  private initializeDefaultStates(): void {
    ...
    this.stateConfigs.set('reviewing', {
      allowedTransitions: ['executing', 'completed', 'failed', 'cancelled'],
    });
   ...
   */
  async transition(to: SessionStatus, action: string = 'unknown'): Promise<boolean> {
   ...
   */
  onStateEnter(state: SessionStatus, callback: () => void | Promise<void>): void {
   ...
   */
  onStateExit(state: SessionStatus, callback: () => void | Promise<void>): void {
    const config = this.stateConfigs.get(state);
    if (config) {
   ...
   */
  onTransition(callback: (from: SessionStatus, to: SessionStatus) => void): string {
